...
MESH-FILE absence is deliberately NOT judged here -- tools/audit/missing_mesh_refs.py
owns that question and consults the BSAs as well as the loose tree.

Usage:
    python tools/validate/refr_render_audit.py --plugin Oblivion.esm
    python tools/validate/refr_render_audit.py --plugin Oblivion.esm --region -10 10 -10 10
    python tools/validate/refr_render_audit.py --plugin Oblivion.esm --cell 13,21
    python tools/validate/refr_render_audit.py --plugin Oblivion.esm --show bucket_mismatch
...
def _inventory(args, path, lookup):
    ...
    from tools.audit.missing_mesh_refs import _records as _model_records
    from tools.audit.missing_mesh_refs import _bsa_meshes
...
def main():
    ...
    print('  python tools/audit/missing_mesh_refs.py --plugin %s' % args.plugin)
