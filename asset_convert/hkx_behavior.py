...
# ---------------------------------------------------------------------------
#
# The claim tables above match on the FILENAME stem, which is Oblivion's
# convention but not its contract.  What actually binds a clip to a behaviour
# is the NiControllerSequence NAME -- the AnimGroup -- stored inside the .kf;
# the CS Animation tab and the engine both read that, and a folder is free to
# spell the file however it likes.
#
# Morrowind_ob does exactly that: nixhound/walkforward.kf declares AnimGroup
# `Forward` and walkfastforward.kf declares `FastForward`, so neither stem
# equals the `forward`/`fastforward` the table expects.  Every gait clip for 8
# creature folders (alit, bettynetch, cliffracer, horker, nixhound, shalk,
# spherecenturion, steamcenturion) fell into the dead `extra` bucket: the
# generated graph had NO MoveForward state at all, so the engine translated
# the actor with the idle pose playing -- "the nix hound slides while moving
# forward", reported in game 2026-08-26.
#
# This runs AFTER every stem claim, never instead of one.  Stems stay
# authoritative because an AnimGroup is deliberately NOT unique: idle.kf,
# handtohandidle.kf and twohandidle.kf all declare `idle` (see
# COMBAT_IDLE_CANDIDATES), so AnimGroup alone cannot tell a stance variant
# from the base clip.  Only slots still EMPTY after the stem pass are filled
# here, and only from clips no stem claim already took.
...
# The run gait, per state: a faster clip that feeds the speed blend rather
# than replacing the walk (mirrors the `run` / `run_back` stem pass).
_ANIMGROUP_RUN = {
...
def read_animgroup(kf_path: str):
    """The NiControllerSequence name of a .kf -- its AnimGroup.

    Header-only read: the name is the first field of block 0, so nothing past
    the header is decoded (0.06 ms/file vs a full pyffi parse).  Layout is
    nif.xml's `Header` compound verbatim -- Export Info and User Version 2
    share the `User Version >= 10` gate, Block Size starts at 20.2.0.7, and
    the string table at 20.1.0.3.  Verified against pyffi's own header read on
    all 3211 creature KFs in the three test plugins: zero mismatches.

    Returns None if the file is not a readable NIF-family stream.
    """
    try:
    ...
    try:
        ...
        if uver >= 10 or (uver == 1 and ver != 0x0A020000):
            o += 4                                   # User Version 2
            for _ in range(3):                       # Export Info
                o += 1 + d[o]
        ntypes, = struct.unpack_from('<H', d, o)
        o += 2
        for _ in range(ntypes):                      # Block Types
            ln, = struct.unpack_from('<I', d, o)
            o += 4 + ln
        o += 2 * nblocks                             # Block Type Index
        if ver >= 0x14020007:
            o += 4 * nblocks                         # Block Size
        if ver >= 0x14010003:
            nstr, = struct.unpack_from('<I', d, o)
            o += 8                                   # Num + Max String Length
            for _ in range(nstr):
                ln, = struct.unpack_from('<I', d, o)
                o += 4 + ln
        o += 4                                       # Unknown Int 2
...
def _claim_by_animgroup(out: dict, kfs: dict, used: set) -> None:
    ...
    """
    # A swim clip declares the SAME AnimGroup as its land twin
    # (swimhandtohandfastforward.kf is literally named `FastForward`), so the
    # land slots must never see one: the murkdweller ships a full land gait
    # set AND a full swim set, and without this its land run gait was filled
    # with swimhandtohandfastforward.kf.  Swim clips are claimed by SWIM_CLIPS
    # on their stems, and _promote_water_native moves them into the land slots
    # for a true water native — both of which run before this fallback.
    unclaimed = sorted(s for s in kfs
                       if s not in used and not s.startswith('swim'))
    if not unclaimed:
        return
    # Prefer the BASE gait over a weapon-stance variant of the same AnimGroup.
    # Oblivion stores one AnimGroup per weapon state and the engine picks by
    # what is drawn, so nixhound walkforward.kf and handtohandforward.kf BOTH
    # declare `Forward`; the graph builds one MoveForward, and the relaxed
    # gait is the right base for it (the stem tables make the same choice —
    # plain 'forward' outranks every prefixed spelling).  Without this,
    # alphabetical order handed the slot to handtohandforward.kf.
    prefixes = [p for p, _stance in ATTACK_STANCE_PREFIXES]
    unclaimed.sort(key=lambda s: (
        next((i for i, p in enumerate(prefixes) if s.startswith(p)),
             -1) + 1, s))
    groups = {}
    ...
    # Walk gaits first so a `fastforward` cannot land in the walk slot while a
    # plain `forward` is still available for it.
    for want_run in (False, True):
        for stem in unclaimed:
            ...
            if not g:
                continue
            is_run = g in _ANIMGROUP_RUN
            if is_run != want_run:
                continue
            if want_run:
                state, slot = _ANIMGROUP_RUN[g]
                # A run gait only makes sense once its walk slot is filled.
    ...
    # A creature whose ONLY forward clip is a fast one still needs to walk:
    # promote it rather than leave MoveForward empty (nixhound would otherwise
    # keep sliding if its walkforward.kf were ever absent).
    for stem in unclaimed:
...
def classify_clips(creature_dir: str) -> dict:
    ...
    # Every stem table has now had its pick.  Anything still unclaimed gets
    # one last chance via its AnimGroup — the authored binding — so a folder
    # that spells a gait `walkforward` still gets a MoveForward state instead
    # of a sliding actor (see _claim_by_animgroup).
